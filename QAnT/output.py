...
class logging():
    ...
    """
    def __init__(self,verbose, debug, instrument,logfn="/Users/tommertens/PycharmProjects/quant/output/algo.err"):
        self.verbose = verbose
        self._type = instrument._type
        self.debug = debug
    ...
    def log_message(self, message, logtype='|log|'):
        ...
        _output = "{0}  {1:12s}  {2:17s} {3}  {4}".format(ts, isin_ticker, self.instrument.name[0:17], logtype, message)
        text_file.write(_output + "\n")
        if self.verbose:
            print(_output)
        text_file.close()
    ...
    def _get_isin_ticker(self):
        if self._type == "index":
            isin_ticker = self.instrument.ticker
        else:
            isin_ticker = self.instrument.isin
        return isin_ticker
...
class plotting:
    ...
    def plot_summary(self):
        ...
        plt.ylim(self.keyratios['TotalStockholdersEquity'].min() * 0.9,
                 self.keyratios['TotalStockholdersEquity'].max() * 1.1)
    ...
    def interactive_summary(self):
        ...
        fig['layout']['yaxis2'].update(
            title='Net Income & Free Cashflow [Mio. {0}]'.format(self.keyratios['currency'][0]))
